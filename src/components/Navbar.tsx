                ...
                return (
                  <Link key={link.to} to={link.to}
                    className={`relative flex items-center gap-1.5 px-3 sm:px-4 py-2 rounded-xl text-sm font-semibold transition-all
                      ${active ? 'bg-ink-900 text-white shadow-soft' : 'text-ink-600 hover:text-ink-900 hover:bg-ink-50'}`}>
              ...
              {user ? (
                <div className="relative ml-1">
                  <button onClick={() => setShowMenu(v => !v)}
                    ...
                    <div className="w-7 h-7 rounded-full bg-gradient-to-br from-saffron to-amber-500 flex items-center justify-center text-white text-xs font-extrabold flex-shrink-0">
                  ...
                  {showMenu && (
                    <>
                      ...
                      <div className="absolute right-0 top-full mt-2 w-48 bg-white rounded-2xl border border-ink-100 shadow-card z-20 overflow-hidden">
                        <Link to="/saved-trips" onClick={() => setShowMenu(false)}
